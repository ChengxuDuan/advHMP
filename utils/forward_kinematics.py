...
def fkl_torch(angles, parent, offset, rotInd, expmapInd):
    ...
    if angles.is_cuda == True:
      p3d = p3d.cuda()
    angles = angles[:, 3:].contiguous().view(-1, 3)
    R = data_utils.expmap2rotmat_torch(angles).view(n, j_n, 3, 3)
    for i in np.arange(1, j_n):
        if parent[i] > 0:# i从2开始， parent[i]比i小1
            R[:, i, :, :] = torch.matmul(R[:, i, :, :].clone(), R[:, parent[i], :, :].clone())
            p3d[:, i, :] = (
                torch.matmul(p3d[0, i, :].clone(), R[:, parent[i], :, :]) + p3d[:, parent[i], :].clone()
            )
    return p3d
